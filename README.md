# Kantan-GUI
## 概要
このスクリプトは、手軽にシンプルなフォームアプリを作成できるPowershellスクリプトです。
...
|Required|文字列|false|trueを指定すると必須バリデーションチェックの対象にする。|
|Validation|文字列|false|trueを指定すると実行ボタン押下時にバリデーションチェックを行う。|
|Return|文字列|false|実行ボタンを押したときに引数や戻り値に含むかどうか。|
...
|Required|文字列|false|trueを指定すると必須バリデーションチェックの対象にする。また、falseにすると空白文字での登録が可能になる。|
|Validation|文字列|false|trueを指定すると実行ボタン押下時にバリデーションチェックを行う。|
|Return|文字列|false|実行ボタンを押したときに引数や戻り値に含むかどうか。|
...
|Default|文字列|""|コンポーネントの初期値。|
|Required|文字列|false|trueを指定すると必須バリデーションチェックの対象にする。|
|DandD|文字列|false|trueを指定するとフォームにファイルをドロップしたときにそのパスを取得する。|
|Validation|文字列|false|trueを指定すると実行ボタン押下時にバリデーションチェックを行う。|
|Return|文字列|false|実行ボタンを押したときに引数や戻り値に含むかどうか。|
...
|Default|文字列|""|コンポーネントの初期値。|
|Required|文字列|false|trueを指定すると必須バリデーションチェックの対象にする。|
|DandD|文字列|false|trueを指定するとフォームにフォルダをドロップしたときにそのパスを取得する。|
|Validation|文字列|false|trueを指定すると実行ボタン押下時にバリデーションチェックを行う。|
|Return|文字列|false|実行ボタンを押したときに引数や戻り値に含むかどうか。|
